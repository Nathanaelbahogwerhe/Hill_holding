    ...
    <div class="bg-hh-card rounded-lg p-6 border border-hh-border">
        ...
        <form action="{{ route('admin.roles.update', $role) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $role->name) }}" required class="w-full px-4 py-2 bg-hh-dark border border-hh-border rounded-lg text-hh-light focus:outline-none focus:ring-2 focus:ring-hh-gold @error('name') border-red-500 @enderror">
                @error('name')
