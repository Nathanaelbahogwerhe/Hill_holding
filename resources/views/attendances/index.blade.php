...
<div class="px-6 py-6">
    ...
    <div class="mb-8">
        ...
        <p class="text-neutral-400">Suivez la présence et les horaires des employés</p>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        ...
        <div class="bg-gradient-to-br from-neutral-900 to-black border border-neutral-800 rounded-2xl p-6 shadow-2xl hover:scale-105 transition-transform duration-300">
            ...
            <div class="text-neutral-400 text-sm">Présents</div>
        </div>
        ...
        <div class="bg-gradient-to-br from-neutral-900 to-black border border-neutral-800 rounded-2xl p-6 shadow-2xl hover:scale-105 transition-transform duration-300">
            ...
            <div class="text-neutral-400 text-sm">Absents</div>
        </div>
        ...
        <div class="bg-gradient-to-br from-neutral-900 to-black border border-neutral-800 rounded-2xl p-6 shadow-2xl hover:scale-105 transition-transform duration-300">
            ...
            <div class="text-neutral-400 text-sm">En retard</div>
        </div>
        ...
        <table class="table-auto w-full">
            <thead>
                <tr class="bg-gradient-to-r from-[#D4AF37]/20 via-yellow-500/20 to-[#D4AF37]/20 border-b border-[#D4AF37]/30">
                    ...
                    <th class="p-4 text-left text-[#D4AF37] font-bold">Arrivée</th>
                    <th class="p-4 text-left text-[#D4AF37] font-bold">Départ</th>
                    ...
                    <tr class="hover:bg-neutral-800/50 transition-colors duration-200">
                        <td class="p-4">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 rounded-full bg-gradient-to-r from-purple-500 to-purple-600 flex items-center justify-center text-white font-bold shadow-lg">
                                    {{ strtoupper(substr($att->employee?->first_name ?? 'U', 0, 1)) }}{{ strtoupper(substr($att->employee?->last_name ?? 'N', 0, 1)) }}
                                </div>
                                <div>
                        ...
                        <td class="p-4">
                            <div class="text-neutral-300 text-sm">{{ $att->employee?->agence?->name ?? '—' }}</div>
                        </td>
                        <td class="p-4">
                        ...
                        <td class="p-4">
                            <div class="inline-flex items-center gap-1 px-3 py-1 bg-green-900/30 border border-green-500/50 rounded-lg">
                                ...
                                <span class="text-green-300 text-sm font-semibold">{{ $att->check_in ?? '—' }}</span>
                            </div>
                        </td>
                        <td class="p-4">
                            @if($att->check_out)
                                <div class="inline-flex items-center gap-1 px-3 py-1 bg-red-900/30 border border-red-500/50 rounded-lg">
                                    ...
                                    <span class="text-red-300 text-sm font-semibold">{{ $att->check_out }}</span>
                                </div>
                            @else
                        ...
                        <td class="p-4">
                            <div class="flex gap-2">
                                ...
                                <form action="{{ route('attendances.destroy', $att->id) }}" method="POST"
                                      onsubmit="return confirm('Supprimer cette présence ?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                    ...
                    <tr>
                        <td colspan="8" class="px-4 py-16 text-center">
                            <div class="flex flex-col items-center gap-4">
                                ...
                                <div class="text-neutral-400 text-lg">Aucune présence trouvée</div>
                                <a href="{{ route('attendances.create') }}"
    ...
    <div class="mt-6">
        <div class="flex justify-between items-center">
            <div class="text-neutral-400">
                Affichage de {{ $attendances->firstItem() ?? 0 }} à {{ $attendances->lastItem() ?? 0 }} sur {{ $attendances->total() }} présences
            </div>
