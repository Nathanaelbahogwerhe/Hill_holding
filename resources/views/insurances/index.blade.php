...
<div class="max-w-7xl mx-auto bg-hh-card p-6 rounded shadow">
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="{{ route('insurances.create') }}" class="btn btn-primary flex items-center space-x-2">
        ...
        <table class="w-full table-auto border border-gray-700 rounded shadow">
            <thead class="bg-gray-800">
                <tr>
                    ...
                    <th class="px-4 py-2 text-left text-hh-gold">Compagnie</th>
                    <th class="px-4 py-2 text-left text-hh-gold">Début</th>
                    <th class="px-4 py-2 text-left text-hh-gold">Fin</th>
                    <th class="px-4 py-2 text-left text-hh-gold">Prime</th>
            ...
            <tbody>
                ...
                <tr class="border-t border-gray-700">
                    ...
                    <td class="px-4 py-2">{{ $insurance->company }}</td>
                    ...
                    <td class="px-4 py-2">{{ number_format($insurance->premium,0,',',' ') }} FBU</td>
                    <td class="px-4 py-2 space-x-2">
                        <a href="{{ route('insurances.show', $insurance) }}" class="text-blue-600 hover:underline">Voir</a>
                        <a href="{{ route('insurances.edit', $insurance) }}" class="text-green-600 hover:underline">Éditer</a>
                        <form action="{{ route('insurances.destroy', $insurance) }}" method="POST" class="inline">
                            @csrf @method('DELETE')
                ...
                @empty
                <tr>
                    <td colspan="7" class="px-4 py-2 text-center text-gray-400">Aucune assurance trouvée.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
