...
@section('content')
<div class="bg-hh-card rounded-xl shadow p-6 max-w-4xl mx-auto">
    ...
    <div class="space-y-4">
        <div>
            ...
            <p class="text-[#D4AF37]">{{ $task->title }}</p>
        </div>

        <div>
            <label class="block font-medium mb-1">Projet</label>
            <p class="text-[#D4AF37]">{{ $task->project->name ?? '—' }}</p>
        </div>

        <div>
            <label class="block font-medium mb-1">Responsable / Assigné à </label>
            <p class="text-[#D4AF37]">{{ $task->assignedTo->name ?? 'Non assigné' }}</p>
        </div>

        <div>
            <label class="block font-medium mb-1">Description</label>
            <p class="text-[#D4AF37] whitespace-pre-line">{{ $task->description ?? '—' }}</p>
        </div>
        ...
        <div>
            ...
            <p class="text-[#D4AF37]">{{ $task->due_date?->format('d/m/Y') ?? '—' }}</p>
        </div>
    </div>
