...
@section('title', 'Nouvelle TÃ¢che')

@section('content')
<div class="bg-hh-card dark:bg-hh-gray-dark rounded-lg shadow p-6 max-w-4xl mx-auto">
    <h2 class="text-xl font-semibold mb-4">ðŸ“ CrÃ©er une nouvelle tÃ¢che</h2>
    ...
    <form action="{{ route('tasks.store') }}" method="POST" class="space-y-4">
        ...
        <div>
            ...
            <select name="status" id="status"
                class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-hh-primary">
                <option value="todo" {{ old('status') == 'todo' ? 'selected' : '' }}>Ã€ faire</option>
                <option value="doing" {{ old('status') == 'doing' ? 'selected' : '' }}>En cours</option>
                <option value="done" {{ old('status') == 'done' ? 'selected' : '' }}>TerminÃ©</option>
            </select>
