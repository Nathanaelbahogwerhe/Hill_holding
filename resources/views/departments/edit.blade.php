@extends('layouts.app')
@section('title', 'Éditer Département')

@section('content')
<div class="max-w-4xl mx-auto px-4 py-6">
    <div class="flex justify-between items-center mb-8">
        <h1 class="text-4xl font-bold text-[#D4AF37]">✏️ Éditer {{ $department->name }}</h1>
        ...
        <form action="{{ route('departments.update', $department->id) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="name" value="{{ $department->name }}" class="w-full px-4 py-2 rounded-xl bg-neutral-900 border border-neutral-700 text-white focus:border-[#D4AF37]" required>
                </div>
                <div>
                    <label class="block mb-2 font-semibold text-[#D4AF37]">Code</label>
                    <input type="text" name="code" value="{{ $department->code }}" class="w-full px-4 py-2 rounded-xl bg-neutral-900 border border-neutral-700 text-white focus:border-[#D4AF37]">
                </div>
                <div>
                    ...
                    <select name="filiale_id" id="filiale_id" class="w-full px-4 py-2 rounded-xl bg-neutral-900 border border-neutral-700 text-white focus:border-[#D4AF37]">
                        ...
                        @foreach($filiales as $filiale)
                            <option value="{{ $filiale->id }}" {{ $department->filiale_id == $filiale->id ? 'selected' : '' }}>{{ $filiale->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    <label class="block mb-2 font-semibold text-[#D4AF37]">Agence</label>
                    <select name="agency_id" id="agency_id" class="w-full px-4 py-2 rounded-xl bg-neutral-900 border border-neutral-700 text-white focus:border-[#D4AF37]">
                        <option value="">-- Sélectionner --</option>
                        @foreach($agences as $agence)
                            <option value="{{ $agence->id }}" data-filiale="{{ $agence->filiale_id }}" {{ $department->agency_id == $agence->id ? 'selected' : '' }}>
            ...
            <div class="flex gap-4 mt-8">
                ...
                <a href="{{ route('departments.index') }}" class="px-6 py-3 bg-neutral-700 hover:bg-neutral-600 text-white rounded-xl font-bold transition">❌ Annuler</a>
            </div>
        </form>
    </div>
</div>
