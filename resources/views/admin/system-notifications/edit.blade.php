        ...
        <form method="POST" action="{{ route('admin.system-notifications.update', $systemNotification) }}" class="space-y-6">
            ...
            <div>
                ...
                <select id="type" name="type" required class="w-full px-4 py-2 bg-hh-dark border border-hh-border rounded-lg text-hh-light focus:outline-none focus:ring-2 focus:ring-hh-gold @error('type') border-red-500 @enderror">
                    ...
                    <option value="error" {{ old('type', $systemNotification->type) == 'error' ? 'selected' : '' }}>Erreur</option>
                </select>
                @error('type')
                <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            ...
            <div id="role-field" style="display: {{ old('target', $systemNotification->target) == 'specific_role' ? 'block' : 'none' }}">
                ...
                <select id="role_name" name="role_name" class="w-full px-4 py-2 bg-hh-dark border border-hh-border rounded-lg text-hh-light focus:outline-none focus:ring-2 focus:ring-hh-gold @error('role_name') border-red-500 @enderror">
                    ...
                    <option value="Admin Finance" {{ old('role_name', $systemNotification->role_name) == 'Admin Finance' ? 'selected' : '' }}>Admin Finance</option>
                    <option value="Operations Manager" {{ old('role_name', $systemNotification->role_name) == 'Operations Manager' ? 'selected' : '' }}>Operations Manager</option>
                    <option value="Chargé des Opérations" {{ old('role_name', $systemNotification->role_name) == 'Chargé des Opérations' ? 'selected' : '' }}>Chargé des Opérations</option>
                </select>
                @error('role_name')
                <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            ...
            <div class="flex items-center gap-3">
                <input type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', $systemNotification->is_active) ? 'checked' : '' }} class="w-4 h-4 rounded border-hh-border bg-hh-dark text-hh-gold focus:ring-2 focus:ring-hh-gold">
