@extends('layouts.app')
@section('title', 'Ã‰diter congÃ©')

@section('content')
<div class="max-w-xl mx-auto bg-hh-card p-6 rounded shadow">
    <h2 class="text-lg font-semibold mb-4">Ã‰diter le congÃ©</h2>
    ...
    <form action="{{ route('leaves.update', $leave) }}" method="POST">
        ...
        <div class="mb-4">
            <label class="block mb-1 font-semibold">Type de congÃ©</label>
            <select name="leave_type_id" class="input input-bordered w-full" required>
        ...
        <div class="mb-4">
            ...
            <input type="date" name="start_date" value="{{ old('start_date', $leave->start_date->format('Y-m-d')) }}" class="input input-bordered w-full" required>
        </div>

        <div class="mb-4">
            <label class="block mb-1 font-semibold">Date fin</label>
            <input type="date" name="end_date" value="{{ old('end_date', $leave->end_date->format('Y-m-d')) }}" class="input input-bordered w-full" required>
        </div>
