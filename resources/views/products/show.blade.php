...
<div class="max-w-4xl mx-auto px-4 py-6">
    ...
    <div class="flex justify-between items-center mb-8">
        ...
        <div class="space-x-2">
            <a href="{{ route('products.edit', $product->id) }}" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-bold">
                 Éditer
            </a>
    ...
    <div class="bg-slate-900 rounded-lg shadow-xl p-8 border border-slate-700 mb-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <p class="text-lg text-gray-100">{{ $product->id }}</p>
            </div>
            
            <div>
                <p class="text-blue-400 text-sm uppercase font-semibold mb-1">Nom</p>
                <p class="text-lg text-gray-100">{{ $product->name ?? $product->first_name ?? 'N/A' }}</p>
            </div>
            
            <div>
                <p class="text-blue-400 text-sm uppercase font-semibold mb-1">Créé le</p>
                <p class="text-lg text-gray-100">{{ $product->created_at?->format('d/m/Y H:i') ?? 'N/A' }}</p>
            </div>
            
            <div>
                <p class="text-blue-400 text-sm uppercase font-semibold mb-1">Mis à jour le</p>
                <p class="text-lg text-gray-100">{{ $product->updated_at?->format('d/m/Y H:i') ?? 'N/A' }}</p>
            </div>
        </div>
    </div>
