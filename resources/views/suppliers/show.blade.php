...
<div class="px-6 py-6">
    ...
    <div class="mb-8">
        <div class="flex justify-between items-center">
            <div>
                ...
                <p class="text-neutral-400">Informations complètes du fournisseur</p>
            </div>
    ...
    <div class="bg-gradient-to-br from-neutral-900 to-black border border-neutral-800 rounded-2xl shadow-2xl p-8 mb-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div class="bg-neutral-800/50 rounded-xl p-6 border border-neutral-700">
                ...
                <p class="text-2xl font-bold text-white">{{ $supplier->id }}</p>
            </div>
            ...
            <div class="bg-neutral-800/50 rounded-xl p-6 border border-neutral-700">
                ...
                <p class="text-2xl font-bold text-white">{{ $supplier->name ?? $supplier->first_name ?? 'N/A' }}</p>
            </div>
            ...
            <div class="bg-neutral-800/50 rounded-xl p-6 border border-neutral-700">
                ...
                <p class="text-lg text-white">{{ $supplier->created_at?->format('d/m/Y H:i') ?? 'N/A' }}</p>
                <p class="text-xs text-neutral-500 mt-1">{{ $supplier->created_at?->diffForHumans() }}</p>
            </div>
            ...
            <div class="bg-neutral-800/50 rounded-xl p-6 border border-neutral-700">
                ...
                <p class="text-lg text-white">{{ $supplier->updated_at?->format('d/m/Y H:i') ?? 'N/A' }}</p>
                <p class="text-xs text-neutral-500 mt-1">{{ $supplier->updated_at?->diffForHumans() }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-gradient-to-br from-red-900/20 to-red-800/20 border border-red-500/30 rounded-2xl shadow-xl p-6">
        <div class="flex items-center justify-between">
            ...
            <form action="{{ route('suppliers.destroy', $supplier->id) }}" method="POST" onsubmit="return confirm('Confirmer la suppression définitive?')">
                @csrf
                @method('DELETE')
                <button type="submit" 
