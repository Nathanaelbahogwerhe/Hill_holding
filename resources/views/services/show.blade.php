...
<div class="max-w-4xl mx-auto px-4 py-6">
    ...
    <div class="flex justify-between items-center mb-8">
        ...
        <div class="space-x-2">
            <a href="{{ route('services.edit', $service->id) }}" class="px-4 py-2 bg-gradient-to-r from-[#D4AF37] to-yellow-500 hover:from-yellow-500 hover:to-[#D4AF37] text-black rounded-xl font-bold text-white rounded-xl font-bold">
                 Éditer
            </a>
    ...
    <div class="bg-slate-900 rounded-xl shadow-xl p-8 border border-slate-700 mb-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <p class="text-lg text-white">{{ $service->id }}</p>
            </div>
            
            <div>
                <p class="text-blue-400 text-sm uppercase font-semibold mb-1">Nom</p>
                <p class="text-lg text-white">{{ $service->name ?? $service->first_name ?? 'N/A' }}</p>
            </div>
            
            <div>
                <p class="text-blue-400 text-sm uppercase font-semibold mb-1">Créé le</p>
                <p class="text-lg text-white">{{ $service->created_at?->format('d/m/Y H:i') ?? 'N/A' }}</p>
            </div>
            
            <div>
                <p class="text-blue-400 text-sm uppercase font-semibold mb-1">Mis à jour le</p>
                <p class="text-lg text-white">{{ $service->updated_at?->format('d/m/Y H:i') ?? 'N/A' }}</p>
            </div>
        </div>
    </div>
