...
<div class="mb-6">
    ...
    <input 
        ...
        name="attachments[]" 
        multiple 
        class="w-full px-4 py-3 bg-hh-dark border border-hh-border rounded-xl text-hh-light focus:border-hh-accent focus:ring-2 focus:ring-hh-accent/20 transition-all file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:bg-hh-accent file:text-white file:cursor-pointer hover:file:bg-hh-accent/90"
...
<div class="mb-6">
    ...
    <div class="space-y-2">
        ...
        <div class="flex items-center justify-between p-3 bg-hh-dark/30 border border-hh-border rounded-xl hover:bg-hh-dark/50 transition-colors">
            ...
            <div class="flex items-center gap-2 flex-shrink-0">
                ...
                <a href="{{ route($route . '.attachments.download', [$model->id, $index]) }}" 
                   class="px-3 py-1.5 text-xs font-medium text-blue-400 hover:text-blue-300 hover:bg-gradient-to-br from-blue-900/50 to-blue-800/50 border border-blue-500/300/10 rounded-xl transition-colors"
                   title="Télécharger">
                ...
                <form method="POST" action="{{ route($route . '.attachments.delete', [$model->id, $index]) }}" 
                    ...
                    @method('DELETE')
                    <button type="submit" 
                            class="px-3 py-1.5 text-xs font-medium text-red-400 hover:text-red-300 hover:bg-red-500/10 rounded-xl transition-colors"
                            title="Supprimer">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
