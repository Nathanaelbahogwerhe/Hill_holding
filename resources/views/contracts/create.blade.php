...
<div class="max-w-xl mx-auto bg-hh-card p-6 rounded shadow">
    <h2 class="text-lg font-semibold mb-4">Cr̩er un contrat</h2>
    ...
    <form action="{{ route('contracts.store') }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <select name="status" class="input input-bordered w-full">
                ...
                <option value="expired" {{ old('status')=='expired' ? 'selected' : '' }}>Expir̩</option>
                <option value="terminated" {{ old('status')=='terminated' ? 'selected' : '' }}>R̩sili̩</option>
            </select>
        </div>
