        ...
        <form action="{{ route('contracts.update', $contract->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" value="{{ $contract->employee?->first_name }} {{ $contract->employee?->last_name }}" class="w-full px-4 py-2 rounded-lg bg-neutral-900 border border-neutral-700 text-neutral-400" disabled>
                </div>

                <div>
                    <label class="block mb-2 font-semibold text-[#D4AF37]">Filiale</label>
                    <input type="text" value="{{ $contract->employee?->filiale?->name ?? 'Maison Mère' }}" class="w-full px-4 py-2 rounded-lg bg-neutral-900 border border-neutral-700 text-neutral-400" disabled>
                </div>
                ...
                <div>
                    ...
                    <input type="text" value="{{ $contract->employee?->department?->name ?? '—' }}" class="w-full px-4 py-2 rounded-lg bg-neutral-900 border border-neutral-700 text-neutral-400" disabled>
                </div>

                <div>
                    <label class="block mb-2 font-semibold text-[#D4AF37]">Date de début <span class="text-red-500">*</span></label>
                    <input type="date" name="start_date" value="{{ $contract->start_date?->format('Y-m-d') }}" class="w-full px-4 py-2 rounded-lg bg-neutral-900 border border-neutral-700 text-white focus:border-[#D4AF37]" required>
                </div>

                <div>
                    <label class="block mb-2 font-semibold text-[#D4AF37]">Date de fin</label>
                    <input type="date" name="end_date" value="{{ $contract->end_date?->format('Y-m-d') }}" class="w-full px-4 py-2 rounded-lg bg-neutral-900 border border-neutral-700 text-white focus:border-[#D4AF37]">
                </div>

                <div>
                    <label class="block mb-2 font-semibold text-[#D4AF37]">Salaire <span class="text-red-500">*</span></label>
                    <input type="number" name="salary" value="{{ $contract->salary }}" class="w-full px-4 py-2 rounded-lg bg-neutral-900 border border-neutral-700 text-white focus:border-[#D4AF37]" min="0" step="0.01" required>
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" rows="4" class="w-full px-4 py-2 rounded-lg bg-neutral-900 border border-neutral-700 text-white focus:border-[#D4AF37]">{{ $contract->description }}</textarea>
                </div>
