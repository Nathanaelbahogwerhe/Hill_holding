...
<div class="max-w-3xl mx-auto py-6">
    ...
    <div class="bg-hh-card p-6 rounded-xl shadow border border-hh-border">
        ...
        <h2 class="text-xl font-semibold text-hh-gold mb-4">âœï¸ Modifier DÃ©pense</h2>
        ...
        <form action="{{ route('expenses.update', $expense) }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <input type="text" name="title" value="{{ $expense->title }}" class="hh-input" required>
            </div>

            <div>
                <label class="hh-label">Montant</label>
                <input type="number" step="0.01" name="amount"
                       value="{{ $expense->amount }}" class="hh-input" required>
            </div>

            <div>
                <label class="hh-label">Date</label>
                <input type="date" name="date"
                       value="{{ $expense->date }}" class="hh-input" required>
            </div>
            ...
            <div>
                ...
                <select name="filiale_id" class="hh-input">
                    @foreach($filiales as $filiale)
                        <option value="{{ $filiale->id }}" {{ $expense->filiale_id == $filiale->id ? 'selected' : '' }}>
                            {{ $filiale->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="agence_id" class="hh-input">
                    @foreach($agences as $agence)
                        <option value="{{ $agence->id }}" {{ $expense->agence_id == $agence->id ? 'selected' : '' }}>
                            {{ $agence->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <textarea name="description" rows="3"
                          class="hh-input">{{ $expense->description }}</textarea>
            </div>
