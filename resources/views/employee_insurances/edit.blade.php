...
<div class="max-w-xl mx-auto bg-hh-card p-6 rounded shadow">
    <h2 class="text-lg font-semibold mb-4">Ã‰diter lâ€™assurance</h2>
    ...
    <form action="{{ route('employee_insurances.update', $insurance) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="mb-4">
            <label class="block mb-1 font-semibold">NumÃ©ro de police</label>
            <input type="text" name="policy_number" value="{{ old('policy_number', $insurance->policy_number) }}" class="input input-bordered w-full" required>
        </div>
        ...
        <div class="mb-4">
            ...
            <input type="date" name="end_date" value="{{ old('end_date', $insurance->end_date?->format('Y-m-d')) }}" class="input input-bordered w-full">
        </div>
