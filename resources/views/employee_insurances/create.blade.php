...
<div class="max-w-xl mx-auto bg-hh-card p-6 rounded shadow">
    <h2 class="text-lg font-semibold mb-4">Ajouter une assurance</h2>
    ...
    <form action="{{ route('employee_insurances.store') }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="policy_number" value="{{ old('policy_number') }}" class="input input-bordered w-full" required>
        </div>

        <div class="mb-4">
            <label class="block mb-1 font-semibold">Date d̩but</label>
            <input type="date" name="start_date" value="{{ old('start_date', now()->format('Y-m-d')) }}" class="input input-bordered w-full" required>
        </div>
