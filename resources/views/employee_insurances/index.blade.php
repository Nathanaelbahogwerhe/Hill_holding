...
<div class="px-6 py-6">
    ...
    <div class="mb-8">
        ...
        <p class="text-neutral-400">Gérez les assurances et couvertures de vos employés</p>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        ...
        <div class="bg-gradient-to-br from-neutral-900 to-black border border-neutral-800 rounded-2xl p-6 shadow-2xl hover:scale-105 transition-transform duration-300">
            ...
            <div class="text-4xl font-bold bg-gradient-to-r from-green-400 to-green-500 bg-clip-text text-transparent mb-2">
                {{ $insurances->filter(fn($i) => $i->start_date <= now() && (!$i->end_date || $i->end_date >= now()))->count() }}
            </div>
            <div class="text-neutral-400 text-sm">Assurances actives</div>
        </div>
        ...
        <div class="bg-gradient-to-br from-neutral-900 to-black border border-neutral-800 rounded-2xl p-6 shadow-2xl hover:scale-105 transition-transform duration-300">
            ...
            <div class="text-4xl font-bold bg-gradient-to-r from-blue-400 to-blue-500 bg-clip-text text-transparent mb-2">
                {{ $insurances->unique('insurance_plan_id')->count() }}
            </div>
            <div class="text-neutral-400 text-sm">Plans différents</div>
        </div>
    ...
    <div class="bg-gradient-to-br from-neutral-900 to-black border border-neutral-800 rounded-2xl p-6 shadow-2xl mb-6">
        <form method="GET" class="grid grid-cols-1 md:grid-cols-5 gap-4">
            ...
            <div>
                ...
                <select name="insurance_plan_id" 
                        class="w-full px-4 py-2 bg-neutral-800 border border-neutral-700 rounded-xl text-white focus:outline-none focus:border-[#D4AF37] focus:ring-2 focus:ring-[#D4AF37]/20 transition-all">
                    <option value="">Tous</option>
                    @foreach(\App\Models\InsurancePlan::all() as $plan)
                        <option value="{{ $plan->id }}" {{ request('insurance_plan_id') == $plan->id ? 'selected' : '' }}>
                            {{ $plan->name }}
                        </option>
                    @endforeach
                </select>
    ...
    <div class="flex justify-between items-center mb-6">
        <div class="flex gap-3">
            <a href="{{ route('employee_insurances.create') }}" 
            ...
            <table class="w-full">
                <thead>
                    <tr class="bg-gradient-to-r from-[#D4AF37] to-yellow-500">
                        ...
                        <th class="px-6 py-4 text-left text-sm font-bold text-black">Type</th>
                        <th class="px-6 py-4 text-left text-sm font-bold text-black">Date Début</th>
                        <th class="px-6 py-4 text-left text-sm font-bold text-black">Date Fin</th>
                ...
                <tbody class="divide-y divide-neutral-800">
                    @forelse($insurances as $insurance)
                        @php
                            $isActive = $insurance->start_date <= now() && (!$insurance->end_date || $insurance->end_date >= now());
                        @endphp
                        <tr class="hover:bg-neutral-800/50 transition-all duration-200 group">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <div class="w-10 h-10 rounded-full bg-gradient-to-r from-[#D4AF37] to-yellow-500 flex items-center justify-center text-black font-bold">
                                        {{ strtoupper(substr($insurance->employee?->first_name ?? 'U', 0, 1)) }}
                                    </div>
                                    <div>
                            ...
                            <td class="px-6 py-4">
                                <span class="bg-gradient-to-r from-indigo-900/50 to-indigo-800/50 border border-indigo-500/30 text-indigo-300 px-3 py-1 rounded-full text-xs font-semibold">
                            ...
                            <td class="px-6 py-4">
                                <span class="bg-gradient-to-r from-purple-900/50 to-purple-800/50 border border-purple-500/30 text-purple-300 px-3 py-1 rounded-full text-xs font-semibold">
                                    {{ $insurance->insurancePlan?->type ?? '—' }}
                                </span>
                            </td>
                            <td class="px-6 py-4">
                                <span class="text-sm text-neutral-300">
                                    {{ optional($insurance->start_date)->format('d/m/Y') ?? '—' }}
                                </span>
                            </td>
                            <td class="px-6 py-4">
                                <span class="text-sm text-neutral-300">
                                    {{ optional($insurance->end_date)->format('d/m/Y') ?? '—' }}
                                </span>
                            </td>
                            <td class="px-6 py-4">
                                @if($isActive)
                                    <span class="bg-gradient-to-r from-green-900/50 to-green-800/50 border border-green-500/30 text-green-300 px-3 py-1 rounded-full text-xs font-semibold">
                                        ✓ Actif
                                    </span>
                                @else
                                    <span class="bg-gradient-to-r from-red-900/50 to-red-800/50 border border-red-500/30 text-red-300 px-3 py-1 rounded-full text-xs font-semibold">
                                        ✗ Expiré
                                    </span>
                                @endif
                            </td>
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <a href="{{ route('employee_insurances.show', $insurance->id) }}" 
                                    ...
                                    <form action="{{ route('employee_insurances.destroy', $insurance->id) }}" method="POST" class="inline" onsubmit="return confirm('Confirmer la suppression?')">
                                        @csrf @method('DELETE')
                                        <button type="submit" 
                        ...
                        <tr>
                            <td colspan="8" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center justify-center gap-4">
                                    ...
                                    <div class="text-neutral-400">Aucune assurance trouvée</div>
                                    <a href="{{ route('employee_insurances.create') }}" 
