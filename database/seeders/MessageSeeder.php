...
class MessageSeeder extends Seeder
{
    public function run(): void
    {
        ...
        if ($employees->count() < 2) {
            $this->command->warn('â›” MessageSeeder : au moins 2 employÃ©s sont requis.');
            return;
        }

        // --- 1) Messages locaux : au sein de la mÃªme agence ---
        ...
        foreach ($employees as $sender) {
            foreach ($employees as $recipient) {
                if ($sender->id === $recipient->id) continue; // ignorer soi-mÃªme
                if ($sender->agency_id === $recipient->agency_id) continue; // dÃ©jÃ  couvert dans local

                DB::table('messages')->updateOrInsert(
                    [
                        'sender_id'    => $sender->id,
                        'recipient_id' => $recipient->id,
                        'subject'      => "Message de {$sender->name} Ã  {$recipient->name}",
                    ],
                    [
        ...
        $this->command->info('ðŸ“© MessageSeeder : messages locaux et globaux crÃ©Ã©s avec succÃ¨s !');
    }
}
